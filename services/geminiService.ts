...
import { AnalysisRequest, AnalysisResult, Product, Supplier, AnalysisProgress, ContractDetails, PotentialScore, AdditionalCosts, BidRecommendation, QuickSearchResult } from '../types';
        ...
        products: {
            ...
            items: {
                ...
                properties: {
                    ...
                    features: { type: Type.STRING, description: "A DETAILED concatenation of all descriptions, characteristics, and technical requirements." },
                ...
                required: ["id", "itemType", "name", "positionNumber", "parentProductName", "ifxtCode", "manufacturer", "features", "unit", "quantity", "startPrice"]
            }
        }
    },
...
const getDataExtractionPrompt = (request: AnalysisRequest, knowledgeBaseContent?: string): string => {
    const hasPrimaryDocument = request.content?.includes('**PRIMARY SOURCE');
...
` : '';
    ...
    const auctionInstructions = (request.tenderType === 'Auksion' && hasPrimaryDocument) ? `
...
**CORE LOGIC: PSEUDOCODE EXECUTION**
You are a program that MUST execute the following logic step-by-step. Failure to follow this is a critical error.

1.  **INITIALIZE** \`final_products_list = []\`.

2.  **DEFINE SOURCES:**
    - Let \`primary_source\` be the text under the header containing "**PRIMARY SOURCE**".
    - Let \`fallback_source\` be the text under the header containing "**FALLBACK SOURCE**".

3.  **ATTEMPT PRIMARY EXTRACTION:**
    - If \`primary_source\` is not empty:
        - Analyze ONLY the \`primary_source\` text.
        - Extract all product/service items you find into a temporary list.
        - \`final_products_list = temporary list\`.

4.  **EXECUTE MANDATORY FALLBACK:**
    - If \`final_products_list\` is EMPTY (meaning step 3 found nothing or \`primary_source\` was empty) AND \`fallback_source\` is not empty:
        - Now, analyze ONLY the \`fallback_source\` text.
        - Extract all product/service items you find into a temporary list.
        - \`final_products_list = temporary list\`.

5.  **EXTRACT DEADLINE:**
    - To find the deadline, prioritize the \`fallback_source\` (web page) if it exists, otherwise use the \`primary_source\`. Find the most likely deadline and format it as 'YYYY-MM-DD HH:mm:ss'. If not found, use 'N/A'. Let this be \`final_deadline\`.

6.  **FINAL VERIFICATION (ABSOLUTE RULE):**
    - Look at the raw text of both \`primary_source\` and \`fallback_source\`.
    - If a human can clearly see a list of products in EITHER source, but your \`final_products_list\` is EMPTY, you have made a mistake.
    - In this case, you MUST re-run your analysis on the correct source (whichever one has the list) and populate \`final_products_list\` before generating the output.
    - An empty product list in the final JSON is ONLY acceptable if BOTH sources genuinely contain no product information.

7.  **OUTPUT JSON:**
    - Generate a single JSON object: \`{ "deadline": final_deadline, "products": final_products_list }\`.
    - This JSON must strictly adhere to the provided schema. No extra text.
`;

    return `You are a meticulous procurement data extraction robot. Your SOLE task is to analyze the provided tender document(s) and extract ALL product positions and the lot deadline into a valid JSON object.
...
After following the core logic, ensure your extraction for individual products follows these rules. This applies to whichever source you use (Primary or Fallback).
...
export const analyzeLot = async (
    ...
    try {
        ...
        // Step 1: Unified content gathering from URL, with deep dive for linked documents.
        if (request.url) {
            try {
                ...
                if (pageResponse.ok) {
                    ...
                    const techSpecKeywords = [
                        'texnik topshiriq', 'техническое задание', 'technical specification', 
                        'тех. задание', 'спецификация', 'техзадание', 'техзадания',
                        'техническая документация', 'тех. документация'
                    ];
                    ...
                    for (const link of allLinks) {
                        ...
                        const combinedText = `${linkText} ${linkHref}`;

                        if (techSpecKeywords.some(keyword => combinedText.includes(keyword))) {
                             if (linkHref && (linkHref.endsWith('.pdf') || linkHref.endsWith('.docx') || linkHref.includes('download'))) {
                                techSpecFileUrl = new URL(linkHref, request.url).href;
                                break; 
                            }
                        }
                    }
                    ...
                    if (techSpecFileUrl) {
                        console.log("Found potential specification file, attempting to fetch:", techSpecFileUrl);
                        fetchedFileContent = await fetchAndExtractFile(techSpecFileUrl);
                    }
                }
            } catch (e) {
                console.error(`Failed to auto-fetch and process from URL ${request.url}, proceeding with provided data.`, e);
            }
        }
        
        // Step 2: Build the prompt and history content using a standardized structure.
        const fileSource = fetchedFileContent || userUploadedContent;
        const htmlSource = fetchedHtmlContent;

        let primarySourceHeader = '';
        let primarySourceContent = '';
        let fallbackSourceHeader = '';
        let fallbackSourceContent = '';

        if (fileSource) {
            primarySourceHeader = fetchedFileContent ? '**PRIMARY SOURCE (Linked Document):**\n' : '**PRIMARY SOURCE (Uploaded Document):**\n';
            primarySourceContent = fileSource;
            if (htmlSource) {
                fallbackSourceHeader = '**FALLBACK SOURCE (Web Page Content):**\n';
                fallbackSourceContent = htmlSource;
            }
        } else if (htmlSource) {
            primarySourceHeader = '**PRIMARY SOURCE (Web Page Content):**\n';
            primarySourceContent = htmlSource;
        } else {
            throw new Error("No content could be loaded for analysis.");
        }

        contentForPrompt = `${primarySourceHeader}${primarySourceContent}`;
        if (fallbackSourceContent) {
            contentForPrompt += `\n\n${fallbackSourceHeader}${fallbackSourceContent}`;
        }
        fullContentForHistory = contentForPrompt;

        const enrichedRequest = { ...request, content: contentForPrompt.trim() };
